...
def process_gowalla_dataset():
    ...
    data["norm_in_day_time"] = (data["timestamp"].dt.hour * 3600 + data["timestamp"].dt.minute * 60 + data["timestamp"].dt.second) / 86400
    data["norm_day_shift"] = data.groupby("user_id")["timestamp"].diff().dt.total_seconds() / 86400
    data["norm_relative_time"] = data.groupby("user_id")["norm_in_day_time"].diff()
    ...
    X = data[["user_id", "poi_id", "latitude", "longitude", "trajectory_id", "day_of_week", "norm_in_day_time", "norm_day_shift", "norm_relative_time"]]
